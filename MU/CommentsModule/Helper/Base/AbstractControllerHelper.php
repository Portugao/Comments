...
abstract class AbstractControllerHelper
{
    ...
    public function processViewActionParameters($objectType, SortableColumns $sortableColumns, array $templateParameters = [], $hasHookSubscriber = false)
    {
        ...
        $templateParameters['own'] = (bool)$request->query->getInt('own', $this->variableApi->get('MUCommentsModule', 'showOnlyOwnEntries', false)) ? 1 : 0;
        ...
        if ($templateParameters['all'] != 1) {
            ...
            if (in_array($resultsPerPage, [0, 10])) {
                $resultsPerPage = $this->variableApi->get('MUCommentsModule', $objectType . 'EntriesPerPage', 10);
            }
        }
        ...
        $quickNavForm = $this->formFactory->create('MU\CommentsModule\Form\Type\QuickNavigation\\' . ucfirst($objectType) . 'QuickNavType', $templateParameters);
        ...
        if (true === $hasHookSubscriber) {
            ...
            $templateParameters['currentUrlObject'] = new RouteUrl('mucommentsmodule_' . strtolower($objectType) . '_view', $urlParameters);
        }
    ...
    public function processDisplayActionParameters($objectType, array $templateParameters = [], $hasHookSubscriber = false)
    {
        ...
        if (true === $hasHookSubscriber) {
            ...
            $templateParameters['currentUrlObject'] = new RouteUrl('mucommentsmodule_' . strtolower($objectType) . '_display', $urlParameters);
        }
    
        if (in_array($objectType, ['comment'])) {
            $qb = $this->entityFactory->getObjectManager()->createQueryBuilder();
            $qb->select('tbl')
               ->from('MU\CommentsModule\Entity\HookAssignmentEntity', 'tbl')
