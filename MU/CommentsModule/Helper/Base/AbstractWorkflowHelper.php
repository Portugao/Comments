...
abstract class AbstractWorkflowHelper
{
    ...
    public function collectAmountOfModerationItems()
    {
        ...
        $state = 'waiting';
        $objectType = 'comment';
        if ($this->permissionApi->hasPermission('MUCommentsModule:' . ucfirst($objectType) . ':', '::', ACCESS_ADD)) {
            $amount = $this->getAmountOfModerationItems($objectType, $state);
            if ($amount > 0) {
                $amounts[] = [
                    'aggregateType' => 'commentsApproval',
                    'description' => $this->translator->__('Comments pending approval'),
                    'amount' => $amount,
                    'objectType' => $objectType,
                    'state' => $state,
                    'message' => $this->translator->_fn('One comment is waiting for approval.', '%amount% comments are waiting for approval.', $amount, ['%amount%' => $amount])
                ];
        
                $this->logger->info('{app}: There are {amount} {entities} waiting for approval.', ['app' => 'MUCommentsModule', 'amount' => $amount, 'entities' => 'comments']);
            }
        }
