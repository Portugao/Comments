...
namespace MU\CommentsModule\Form\Handler\Common\Base;
...
use MU\CommentsModule\Helper\HookHelper;
...
abstract class AbstractEditHandler
{
    ...
    public function processForm(array $templateParameters)
    {
        ...
        // default to referer
        $refererSessionVar = 'mucommentsmodule' . $this->objectTypeCapital . 'Referer';
        ...
        $this->permissionComponent = 'MUCommentsModule:' . $this->objectTypeCapital . ':';
        ...
        if (false === $actions || !is_array($actions)) {
            ...
            $logArgs = ['app' => 'MUCommentsModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $entity->getKey()];
        ...
        if ($isRegularAction || $action == 'delete') {
            ...
            if ($entity->supportsHookSubscribers()) {
                ...
                if ($action != 'delete') {
                    ...
                    $routeUrl = new RouteUrl('mucommentsmodule_' . $this->objectTypeLower . '_display', $urlArgs);
                }
    ...
    protected function addDefaultMessage(array $args = [], $success = false)
    {
        ...
        $logArgs = ['app' => 'MUCommentsModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $this->entityRef->getKey()];
        if (true === $success) {
    ...
    public function fetchInputData()
    {
        ...
        if (isset($this->form['additionalNotificationRemarks']) && $this->form['additionalNotificationRemarks']->getData() != '') {
            $this->request->getSession()->set('MUCommentsModuleAdditionalNotificationRemarks', $this->form['additionalNotificationRemarks']->getData());
        }
    ...
    protected function prepareWorkflowAdditions($enterprise = false)
    {
        ...
        $groupApplicationArgs = [
            'user' => $currentUserId,
            'group' => $this->variableApi->get('MUCommentsModule', 'moderationGroupFor' . $this->objectTypeCapital, GroupsConstant::GROUP_ID_ADMIN)
        ];
        ...
        if (true === $enterprise) {
            $groupApplicationArgs = [
                'user' => $currentUserId,
                'group' => $this->variableApi->get('MUCommentsModule', 'superModerationGroupFor' . $this->objectTypeCapital, GroupsConstant::GROUP_ID_ADMIN)
            ];
