 ...
 */
function mUCommentsToggleRelatedItemForm(idPrefix)
{
 ...
 */
function mUCommentsResetRelatedItemForm(idPrefix)
{
    // hide the sub form
    mUCommentsToggleRelatedItemForm(idPrefix);
 ...
 */
function mUCommentsCreateRelationWindowInstance(containerElem, useIframe)
{
    var newWindowId;
 ...
 */
function mUCommentsInitInlineRelationWindow(objectType, containerID)
{
    var found, newItem;
    ...
    jQuery.each(relationHandler, function (key, singleRelationHandler) {
        ...
        if (singleRelationHandler.prefix === containerID) {
            ...
            singleRelationHandler.windowInstanceId = mUCommentsCreateRelationWindowInstance(jQuery('#' + containerID), true);
        }
    });
    ...
    newItem = {
        ...
        acInstance: null,
        windowInstanceId: mUCommentsCreateRelationWindowInstance(jQuery('#' + containerID), true)
    };
 ...
 */
function mUCommentsRemoveRelatedItem(idPrefix, removeId)
{
 ...
 */
function mUCommentsSelectRelatedItem(objectType, idPrefix, selectedListItem)
{
    ...
    removeLink = jQuery('<a />', { id: elemPrefix + 'Remove', href: 'javascript:mUCommentsRemoveRelatedItem(\'' + idPrefix + '\', ' + newItemId + ');', text: 'remove' });
    ...
    if (true === includeEditing) {
        jQuery('#' + elemPrefix + 'Edit').click(function (event) {
            ...
            mUCommentsInitInlineRelationWindow(objectType, idPrefix + 'Reference_' + newItemId + 'Edit');
        });
    }
 ...
 */
function mUCommentsInitRelationItemsForm(objectType, idPrefix, includeEditing)
{
    ...
    jQuery.each(relationHandler, function (key, singleRelationHandler) {
        ...
        jQuery('#' + idPrefix + 'Selector').autocomplete({
            ...
            select: function(event, ui) {
                if (true === isHookAttacher) {
                    mUCommentsSelectHookItem(objectType, idPrefix, ui.item);
                } else {
                    mUCommentsSelectRelatedItem(objectType, idPrefix, ui.item);
                }
    ...
    jQuery('#' + idPrefix + 'SelectorDoNew').click(function (event) {
        event.preventDefault();
        mUCommentsInitInlineRelationWindow(objectType, idPrefix + 'SelectorDoNew');
    });
    ...
    jQuery.each(itemIdsArr, function (key, existingId) {
        ...
        if (existingId) {
            ...
            jQuery('#' + elemPrefix).click(function (event) {
                event.preventDefault();
                mUCommentsInitInlineRelationWindow(objectType, elemPrefix);
            });
        }
    });
}
 ...
 */
function mUCommentsCloseWindowFromInside(idPrefix, itemId, searchTerm)
{
    ...
    jQuery.each(window.parent.relationHandler, function (key, singleRelationHandler) {
        ...
        if (singleRelationHandler.prefix === idPrefix) {
            ...
            if (null !== singleRelationHandler.acInstance) {
                ...
                // show a message
                window.parent.mUCommentsSimpleAlert(selector, window.parent.Translator.__('Information'), window.parent.Translator.__('Action has been completed.'), 'actionDoneAlert', 'success');
