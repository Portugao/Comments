 ...
 */
function mUCommentsCreateInlineEditingWindowInstance(containerElem) {
    var newWindowId;
 ...
 */
function mUCommentsInitInlineEditingWindow(objectType, idPrefix, containerId, inputType) {
    ...
    jQuery.each(mUCommentsInlineEditHandlers, function (key, editHandler) {
        ...
        editHandler.windowInstanceId = mUCommentsCreateInlineEditingWindowInstance(jQuery('#' + containerId));
    });
    ...
    newEditHandler = {
        ...
        inputType: inputType,
        windowInstanceId: mUCommentsCreateInlineEditingWindowInstance(jQuery('#' + containerId))
    };

    // add it to the list of edit handlers
    mUCommentsInlineEditHandlers.push(newEditHandler);
}

/**
 * Creates a link for editing an existing item using inline editing.
 */
function mUCommentsCreateInlineEditLink(objectType, idPrefix, elemPrefix, itemId) {
 ...
 */
function mUCommentsInitInlineEditLink(objectType, idPrefix, elemPrefix, itemId, inputType) {
    jQuery('#' + elemPrefix + 'Edit').click(function (event) {
        event.preventDefault();
        mUCommentsInitInlineEditingWindow(objectType, idPrefix, idPrefix + 'Reference_' + itemId + 'Edit');
    });
}
 ...
 */
function mUCommentsDetermineInputReference(objectType, alias, idPrefix, inputType, targetWindow) {
    var inputPrefix, inputIdentifier, inputField;

    // determine reference to input element
    inputPrefix = targetWindow.jQuery('.mucomments-edit-form').first().attr('name');
    inputField = null;
 ...
 */
function mUCommentsInitInlineEditingButtons(objectType, alias, idPrefix, inputType, createUrl) {
    var inputReference, createButtonId, createButton, itemIds, itemIdsArr;

    inputReference = mUCommentsDetermineInputReference(objectType, alias, idPrefix, inputType, window);
    ...
    if (jQuery('#' + createButtonId).length < 1) {
        ...
        createButton = jQuery('<a />', {
            ...
            class: 'btn btn-default mucommentsmodule-inline-button'
        }).append(
    ...
    createButton.click(function (event) {
        event.preventDefault();
        mUCommentsInitInlineEditingWindow(objectType, idPrefix, createButtonId, inputType);
    });
    ...
    jQuery.each(itemIdsArr, function (key, existingId) {
        ...
        if (existingId) {
            ...
            jQuery('#' + elemPrefix).click(function (event) {
                event.preventDefault();
                mUCommentsInitInlineEditingWindow(objectType, idPrefix, elemPrefix, inputType);
            });
        }
    });
}
 ...
 */
function mUCommentsCloseWindowFromInside(idPrefix, itemId, formattedTitle, searchTerm) {
    ...
    jQuery.each(window.parent.mUCommentsInlineEditHandlers, function (key, editHandler) {
        ...
        inputReference = mUCommentsDetermineInputReference(editHandler.objectType, editHandler.alias, idPrefix, inputType, window.parent);
        if (null === inputReference || null === inputReference.field) {
            return;
        }
        ...
        window.parent.mUCommentsSimpleAlert(anchorElement, window.parent.Translator.__('Information'), window.parent.Translator.__('Action has been completed.'), 'actionDoneAlert', 'success');
