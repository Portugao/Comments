...
abstract class AbstractCommentController extends AbstractController
{
    ...
    protected function viewInternal(Request $request, $sort, $sortdir, $pos, $num, $isAdmin = false)
    {
        ...
        $viewHelper = $this->get('mu_comments_module.view_helper');
        
        $request->query->set('sort', $sort);
        $request->query->set('sortdir', $sortdir);
        $request->query->set('pos', $pos);
        
        $sortableColumns = new SortableColumns($this->get('router'), 'mucommentsmodule_comment_' . ($isAdmin ? 'admin' : '') . 'view', 'sort', 'sortdir');
     ...
     *
     * @param Request $request Current request instance
     * @param integer $id Identifier of treated comment instance
     *
     * @return Response Output
     *
     * @throws AccessDeniedException Thrown if the user doesn't have required permissions
     * @throws NotFoundHttpException Thrown if comment to be displayed isn't found
     */
    public function adminDisplayAction(Request $request, $id)
    {
        $comment = $this->get('mu_comments_module.entity_factory')->getRepository('comment')->selectById($id);
        if (null === $comment) {
            throw new NotFoundHttpException($this->__('No such comment found.'));
        }
    
        return $this->displayInternal($request, $comment, true);
    }
    ...
    public function displayAction(Request $request, $id)
    {
        ...
        return $this->displayInternal($request, $comment, false);
    }
    ...
    protected function displayInternal(Request $request, CommentEntity $comment, $isAdmin = false)
    {
        ...
        $controllerHelper = $this->get('mu_comments_module.controller_helper');
        $templateParameters = $controllerHelper->processDisplayActionParameters($objectType, $templateParameters, $comment->supportsHookSubscribers());
        
        // fetch and return the appropriate template
        $response = $this->get('mu_comments_module.view_helper')->processTemplate($objectType, 'display', $templateParameters);
    ...
    protected function editInternal(Request $request, $isAdmin = false)
    {
        ...
        $formHandler = $this->get('mu_comments_module.form.handler.comment');
     ...
     *
     * @throws AccessDeniedException Thrown if the user doesn't have required permissions
     * @throws NotFoundHttpException Thrown if comment to be deleted isn't found
     * @throws RuntimeException      Thrown if another critical error occurs (e.g. workflow actions not available)
     */
    ...
    protected function deleteInternal(Request $request, $id, $isAdmin = false)
    {
        ...
        $form = $this->createForm(DeletionType::class, $comment);
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemId) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'MUCommentsModule', 'user' => $userName, 'action' => $action, 'entity' => 'comment', 'id' => $itemId, 'errorMessage' => $exception->getMessage()]);
            }
            ...
            if ($action == 'delete') {
                $this->addFlash('status', $this->__('Done! Item deleted.'));
                $logger->notice('{app}: User {user} deleted the {entity} with id {id}.', ['app' => 'MUCommentsModule', 'user' => $userName, 'entity' => 'comment', 'id' => $itemId]);
            } else {
                $this->addFlash('status', $this->__('Done! Item updated.'));
                $logger->notice('{app}: User {user} executed the {action} workflow action for the {entity} with id {id}.', ['app' => 'MUCommentsModule', 'user' => $userName, 'action' => $action, 'entity' => 'comment', 'id' => $itemId]);
            }
            ...
            if ($entity->supportsHookSubscribers()) {
                ...
                if ($action != 'delete') {
                    ...
                    $url = new RouteUrl('mucommentsmodule_comment_display', $urlArgs);
                }
     ...
     * @param integer $id          Identifier of created comment (used for activating auto completion after closing the modal window)
     *
