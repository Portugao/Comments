...
abstract class AbstractExternalController extends AbstractController
{
    ...
    public function displayAction(Request $request, $objectType, $id, $source, $displayMode)
    {
        ...
        $contextArgs = ['controller' => 'external', 'action' => 'display'];
        $templateParameters = $this->get('mu_comments_module.controller_helper')->addTemplateParameters($objectType, $templateParameters, 'controllerAction', $contextArgs);
        
        return $this->render('@MUCommentsModule/External/' . ucfirst($objectType) . '/' . $template, $templateParameters);
    }
    ...
    public function finderAction(Request $request, $objectType, $editor, $sort, $sortdir, $pos = 1, $num = 0)
    {
        ...
        $cssAssetBag->add($assetHelper->resolve('@MUCommentsModule:css/style.css'));
        $cssAssetBag->add([$assetHelper->resolve('@MUCommentsModule:css/custom.css') => 120]);
        
        $listEntriesHelper = $this->get('mu_comments_module.listentries_helper');
        ...
        if (!in_array($objectType, $activatedObjectTypes)) {
            ...
            $redirectUrl = $this->get('router')->generate('mucommentsmodule_external_finder', ['objectType' => array_shift($activatedObjectTypes), 'editor' => $editor]);
        ...
        $form = $this->createForm('MU\CommentsModule\Form\Type\Finder\\' . ucfirst($objectType) . 'FinderType', $templateParameters, $formOptions);
        ...
        if ($searchTerm != '') {
            $qb = $this->get('mu_comments_module.collection_filter_helper')->addSearchFilter($objectType, $qb, $searchTerm);
        }
        ...
        $output = $this->renderView('@MUCommentsModule/External/' . ucfirst($objectType) . '/find.html.twig', $templateParameters);
